...
class SimpleConf(object):
    ...
    def __init__(self, file_name):
        ...
        self.seek_config_file(os.path.dirname(os.path.abspath(file_name) ))
    ...
    @property
    def proj_inc_dirs(self):
        return self.m_proj_inc_dirs
    ...
    def seek_config_file(self, dir_name):
        ...
        for f in files:
            if os.path.basename(f) == '.ycm_simple_conf.xml':
                ...
                return
        if not os.path.samefile(os.path.dirname(dir_name),dir_name):
    ...
    def fetch_default_include_path(self):
        try:
            devnull = open('nul', 'r')
            err_byte = subprocess.check_output(
                    ['clang','-E', '-x', self.m_project_type, '-', '-v'],
                    stdin=devnull,
                    stderr=subprocess.STDOUT
                    )
            pattern = re.compile(
                    '#include \<\.{3}\>.*here\:(.+)End of search list\.',
                    re.DOTALL
                    )
            err = err_byte.decode("utf-8")
    ...
    def find_proj_headers(self ) :
        if not os.path.isdir( self.m_root_dir):
            return [ ]

        if self.m_root_dir in SimpleConf.proj_cache:
            return SimpleConf.proj_cache[ self.m_root_dir ]
        
        logging.warning("searching " + self.m_root_dir)
        projFlags = [ ]
        for root, dirs, files in os.walk(self.m_root_dir):
            for file in files:
                if self.IsHeaderFile(file):
                    projFlags += [ "-I",root ]
                    break
        SimpleConf.proj_cache[self.m_root_dir] = projFlags
        return projFlags
